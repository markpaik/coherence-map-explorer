...
export function createSearch(deps: SearchDeps): SearchHandle {
  ...
  function resultOption(d: SearchDoc, i: number): HTMLLIElement {
    ...
    // Explicit accessible name on the whole option — overrides the
    // concatenated chip+code+title text so AT announces
    // "Grade 8, 8.F.A.3, <title>" rather than "88.F.A.3".
    li.setAttribute(
      "aria-label",
      `${gradeLabel.get(d.grade) ?? gradeAccessibleName(d.grade)}, ${d.code}, ${title.textContent}`,
    );
